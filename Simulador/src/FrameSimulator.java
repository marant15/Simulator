	...
	public void simulation(ArrayList<Regla> reglas, Automata auto, String estadoI, ArrayList<String> pila) {
		start.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (verificarentrada(palabra,auto.alfabetoE)) {
					if (palabra.equals(""))
						palabra = "\u03BB";
					auto.reglas = reglas;
	...
	public void showsteps() {
		...
		for (int i = 0; i < sim.steps.size(); i++) {
			...
			for (int k = 0; k < sim.automata.numerop; k++) {
				...
				int count = 1;
				if (sim.steps.get(i).pila.get(k).length() > 12)
					count = 12;
				else
					count = sim.steps.get(i).pila.get(k).length();
				for (int j = 0; j < sim.steps.get(i).pila.get(k).length() && j < 12; j++) {
					modelo.get(k).setValueAt(
							sim.steps.get(i).pila.get(k).charAt(sim.steps.get(i).pila.get(k).length() - 1 - j),
							12 - count + j, 0);
				}
			}
			try {
